...
int main(int argc, char *argv[])
{
    ...
    if (!nanorq_prepare(&rq, prep_mem, prep_len)) {
        fprintf(stderr, "nanorq_prepare failed\n");
        return 1;
    }

    u32 rows = nanorq_get_pc_rows(&rq);
    u32 SH = nanorq_get_pc_genc_offset(&rq);
    u32 stride = nanorq_recommended_stride(T);
    u32 mem = rows * stride;
    ...
    while (bytes < TEST_BYTES) {
        ...
        if (!nanorq_precalculate(&rq, work_mem, work_len)) {
            fprintf(stderr, "nanorq_precalculate failed\n");
            exit(1);
        }
    ...
    for (u32 rp = 0; rp < (u32)drops; rp++) {
        memcpy(orig_pkts + rp * stride, D_backup + (SH + dropped_esi[rp]) * stride, PAD(T));
    }

    nanorq dec;
    if (!nanorq_encoder_new(K, 0, &dec)) {
        fprintf(stderr, "failed to init decoder\n");
        return 1;
    }

    size_t dec_prep_len = nanorq_calculate_prepare_memory(&dec);
    uint8_t *dec_prep_mem = malloc(dec_prep_len);
    size_t dec_work_len = nanorq_calculate_work_memory(&dec);
    uint8_t *dec_work_mem = malloc(dec_work_len);

    schedule S_dec = {};
    S_dec.ops.a = malloc(sched_bytes);
    S_dec.ops.m = sched_bytes / sizeof(sched_op);
    nanorq_set_op_callback(&dec, &S_dec, ops_push);
    ...
    bytes = 0;
    t0 = usecs();
    double t_prep_dec = 0.0, t_precalc_dec = 0.0, t_ops_run_dec = 0.0, t_ops_mix_dec = 0.0;
    while (bytes < TEST_BYTES) {
        uint64_t t_start = usecs();
        if (!nanorq_prepare(&dec, dec_prep_mem, dec_prep_len)) {
            fprintf(stderr, "nanorq_prepare failed\n");
            exit(1);
        }
        nanorq_set_op_callback(&dec, &S_dec, ops_push);
        for (u32 rp = 0; rp < (u32)drops; rp++) {
            nanorq_replace_symbol(&dec, dropped_esi[rp], K + rp);
        }
        nanorq_patch_matrix(&dec);
        t_prep_dec += (usecs() - t_start) / 1000000.0;

        t_start = usecs();
        S_dec.ops.n = 0;
        S_dec.cpidx = 0;
        if (!nanorq_precalculate(&dec, dec_work_mem, dec_work_len)) {
        ...
        for (u32 rp = 0; rp < (u32)drops; rp++) {
            memcpy(D_dec + (SH + dropped_esi[rp]) * stride, repair_pkts + rp * stride, PAD(T));
        }
        uint64_t t_mid1 = usecs();
        t0 += (t_mid1 - t_mid0);
    ...
    nanorq dec_oh;
    if (!nanorq_encoder_new(K, overhead, &dec_oh)) {
        fprintf(stderr, "failed to init decoder_oh\n");
        return 1;
    }

    size_t dec_oh_prep_len = nanorq_calculate_prepare_memory(&dec_oh);
    uint8_t *dec_oh_prep_mem = malloc(dec_oh_prep_len);
    size_t dec_oh_work_len = nanorq_calculate_work_memory(&dec_oh);
    ...
    nanorq_set_op_callback(&dec_oh, &S_dec_oh, ops_push);

    if (!nanorq_prepare(&dec_oh, dec_oh_prep_mem, dec_oh_prep_len)) {
        fprintf(stderr, "nanorq_prepare failed\n");
        return 1;
    }

    u32 rows_oh = nanorq_get_pc_rows(&dec_oh);
    ...
    while (bytes < TEST_BYTES) {
        ...
        nanorq_set_op_callback(&dec_oh, &S_dec_oh, ops_push);
        for (u32 rp = 0; rp < (u32)drops; rp++) {
            nanorq_replace_symbol(&dec_oh, dropped_esi[rp], K + overhead + rp);
        }
        for (u32 i = 0; i < (u32)overhead; i++) {
            nanorq_replace_symbol(&dec_oh, K + i, K + i);
        }
        nanorq_patch_matrix(&dec_oh);
        t_prep += (usecs() - t_start) / 1000000.0;

        t_start = usecs();
        S_dec_oh.ops.n = 0;
        S_dec_oh.cpidx = 0;
        if (!nanorq_precalculate(&dec_oh, dec_oh_work_mem, dec_oh_work_len)) {
            fprintf(stderr, "decoder precalculate failed\n");
            exit(1);
        }
        ...
        for (u32 rp = 0; rp < (u32)drops; rp++) {
            memcpy(D_dec_oh + (SH + dropped_esi[rp]) * stride, repair_pkts + (overhead + rp) * stride, PAD(T));
        }
        for (u32 i = 0; i < (u32)overhead; i++) {
            memcpy(D_dec_oh + (SH + K + i) * stride, repair_pkts + i * stride, PAD(T));
        }
        uint64_t t_mid1 = usecs();
        t0 += (t_mid1 - t_mid0);
        ...
        t_start = usecs();
        for (u32 rp = 0; rp < (u32)drops; rp++) {
            ops_mix(&dec_oh, D_dec_oh, stride, dropped_esi[rp], decoded_pkts + rp * stride);
        }
