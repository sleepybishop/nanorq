...
static bool decode_phase2(octmat *A, octmat *D, uint16_t i, uint16_t u,
  ...
  for (uint16_t row = row_start; row < row_end; row++) {
    ...
    if (om_A(*A, row, diag) > 1) {
      uint8_t divisor = om_A(*A, row, diag);
      odivrow(om_P(*A), row, A->cols, divisor);
      odivrow(om_P(*D), row, D->cols, divisor);
    }
...
static void decode_phase5(octmat *A, octmat *D, uint16_t i) {
  ...
  for (uint16_t j = 0; j <= i; j++) {
    if (om_A(*A, j, j) != 1) {
      multiple = om_A(*A, j, j);
      odivrow(om_P(*A), j, A->cols, multiple);
      odivrow(om_P(*D), j, D->cols, multiple);
    }
