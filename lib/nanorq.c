...
static struct block_encoder *get_block_encoder(nanorq *rq, uint8_t sbn) {
  ...
  om_resize(&enc->D, P.L + spare, rq->common.T);
...
static void decode_row(params *P, octmat *D, uint32_t row, uint8_t *ptr,
                       size_t len) {
  memset(ptr, 0, len);
...
bool nanorq_generate_symbols(nanorq *rq, uint8_t sbn, struct ioctx *io) {
  ...
  if (S == NULL)
    return false;
  precode_matrix_intermediate(&P, &enc->D, S);
  ...
  if (esi < enc->K) {
    if (enc->inverted) {
      ...
      if (!enc->loaded)
        enc->loaded = load_symbol_matrix(rq, sbn, io);
      if (enc->loaded) {
        memcpy(data, om_R(enc->D, P.S + P.H + esi), enc->D.cols);
        written += enc->D.cols;
      }
    }
  } else {
...
int nanorq_decoder_add_symbol(nanorq *rq, void *data, uint32_t tag,
  ...
  if (esi < dec->K) {
    ...
    memcpy(om_R(dec->D, P.S + P.H + esi), data, dec->D.cols);
