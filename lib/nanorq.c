 ...
 */
bool nanorq_encoder_new(u32 K, u32 overhead, nanorq *rq)
{
    if (!rq) return false;
    if (K < 2 || K > K_max) return false;
    rq->P = params_init(K);
...
uint32_t nanorq_recommended_stride(uint32_t T)
{
    ...
    /* avoid cache-line aliasing: if padded is an exact multiple of 64, add one alignment unit */
    if (padded % 64 == 0)
        padded += align;
    return padded;
}
...
static bool assign_prepare_memory(nanorq *rq, u8 *mem, size_t len)
{
    if (!rq || !mem || len == 0) return false;
    params *P = &rq->P;
    pc *W = &rq->W;
    ...
    a->beg = mem;
    a->end = mem + len;
    if (!u32_vec_init(&W->c, a, W->cols, W->cols, 0)) return false;
    if (!u32_vec_init(&W->ci, a, W->cols, W->cols, 0)) return false;
    if (!u32_vec_init(&W->d, a, W->rows, W->rows, 0)) return false;
    if (!u32_vec_init(&W->di, a, W->rows, W->rows, 0)) return false;
    if (!u32_vec_init(&W->cnz, a, W->cols, W->cols, 0)) return false;
    if (!u32_vec_init(&W->nz, a, W->rows, W->rows, 0)) return false;

    W->NZT = alloc_array(a, u32_vec, 3);
    if (!W->NZT) return false;
    for (u32 i = 1; i < 3; i++)
    if (!u32_vec_init(&W->NZT[i], a, 0, W->rows, 0)) return false;

    W->A = alloc_array(a, u32_vec, W->rows);
    if (!W->A) return false;
    for (u32 i = 0; i < P->S; i++)
    if (!u32_vec_init(&W->A[i], a, 0, snz, 0)) return false;
    for (u32 i = P->S + P->H, esi = 0; i < W->rows; i++, esi++)
    if (!u32_vec_init(&W->A[i], a, 0, GENC_MAX, 0)) return false;
...
{
    if (!rq || !prep_mem || pm_len == 0) return false;
    if (!assign_prepare_memory(rq, prep_mem, pm_len)) return false;
    precode_matrix_gen(&rq->P, &rq->W);
    return precode_matrix_prepare(&rq->P, &rq->W);
}

uint32_t nanorq_get_packet_mix(nanorq *rq, u32 esi, uint32_t *mix_idxs, uint32_t mix_max)
{
    if (!rq || !mix_idxs || mix_max < GENC_MAX) return 0;
    params *P = &rq->P;
    u32 X = esi;
    if (esi >= P->K)
        X += (P->Kprime - P->K);
...
bool nanorq_patch_matrix(nanorq *rq)
{
    if (!rq) return false;
    pc *W = &rq->W;
    if (!W->cnz.a || !W->nz.a || !W->NZT) return false;
...
static bool assign_work_memory(nanorq *rq, u8 *mem, size_t len)
{
    if (!rq || !mem || len == 0) return false;
    params *P = &rq->P;
    pc *W = &rq->W;
    ...
    tmp = W->rows * u_stride;
    if (!u32_vec_init(&W->U, a, tmp, tmp, u_stride)) return false;
    if (!u32_vec_init(&W->F.rowmap, a, W->rows, W->rows, 0)) return false;
    if (!u32_vec_init(&W->F.type, a, W->rows, W->rows, 0)) return false;

    u32 u_aligned = u_stride * 32;
    tmp = 2 * P->H * u_aligned;
    if (!u8_vec_init(&W->UL, a, tmp, tmp, u_aligned)) return false;

    u32 k_aligned = (P->Kprime + P->S + 31) & ~31;
    tmp = P->H * k_aligned;
    if (!u8_vec_init(&W->HDPC, a, tmp, tmp, k_aligned)) return false;
    return true;
}

bool nanorq_precalculate(nanorq *rq, u8 *work_mem, size_t wm_len)
{
    if (!rq || !work_mem || wm_len == 0) return false;
    if (!assign_work_memory(rq, work_mem, wm_len)) return false;
