...
static void ops_apply(uint8_t *D, uint32_t stride, schedule *S, u32 idx)
{
    sched_op op = S->ops.a[idx];
    if (op.u) {
        nanorq_oblas.axpy((D + op.i * stride), (D + op.j * stride), op.u, stride);
    } else {
        nanorq_oblas.scal((D + op.i * stride), op.j, stride);
    }
}
...
void ops_mix(nanorq *rq, uint8_t *D, uint32_t stride, u32 esi, u8 *ptr)
{
    ...
    uint32_t num = nanorq_get_packet_mix(rq, esi, mix, GENC_MAX);
...
void nanorq_get_memory_reqs(uint32_t K, uint32_t overhead, uint32_t stride, struct nanorq_mem_reqs *reqs) {
    ...
    reqs->work_bytes = nanorq_calculate_work_memory(&rq);
    reqs->matrix_bytes = nanorq_get_pc_rows(&rq) * stride;
...
bool nanorq_encode_simple(uint8_t *src_data, uint32_t K, uint16_t T, uint32_t num_repair, uint8_t *repair_out)
{
    ...
    nanorq_get_memory_reqs(K, 0, T, &reqs);

    uint8_t *prep_mem = malloc(reqs.prepare_bytes);
    if (!nanorq_prepare(&rq, prep_mem, reqs.prepare_bytes)) {
        free(prep_mem);
        return false;
    }
    ...
    if (!nanorq_precalculate(&rq, work_mem, reqs.work_bytes)) {
        free(prep_mem);
        free(D);
        free(work_mem);
        free(S_enc.ops.a);
