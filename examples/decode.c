...
int main(int argc, char *argv[]) {
  ...
  for (u32 rp = 0; rp < drops; rp++) {
    ...
    u32 dropped_esi = rp;
    u32 repair_esi = K + rp;
    nanorq_core_replace_symbol(&rq, dropped_esi, repair_esi);

    /* insert repair packet data into d matrix at dropped row */
    nanorq_core_place_symbol(&rq, D, stride, dropped_esi,
                             repair_pkts + rp * stride, PAD(T));
  }

  nanorq_core_patch_matrix(&rq);
